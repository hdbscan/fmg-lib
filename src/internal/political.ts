...
const computeDownhill = (context: GenerationContext): Int32Array => {
  ...
  for (let cellId = 0; cellId < cellCount; cellId += 1) {
    ...
    downhill[cellId] = target < 0 && waterTarget >= 0 ? waterTarget : target;
  }

  return downhill;
};

const computeConfluenceStrength = (
  context: GenerationContext,
  downhill: Int32Array,
): Uint32Array => {
  const { cellCount, cellsFeature, cellsFlow } = context.world;
  const dominantIncoming = new Uint32Array(cellCount);
  const totalIncoming = new Uint32Array(cellCount);

  for (let cellId = 0; cellId < cellCount; cellId += 1) {
    if ((cellsFeature[cellId] ?? 0) !== 1) continue;

    const target = downhill[cellId] ?? -1;
    if (target < 0 || (cellsFeature[target] ?? 0) !== 1) continue;

    const flow = cellsFlow[cellId] ?? 0;
    totalIncoming[target] = (totalIncoming[target] ?? 0) + flow;
    if (flow > (dominantIncoming[target] ?? 0)) {
      dominantIncoming[target] = flow;
    }
  }

  const confluence = new Uint32Array(cellCount);
  for (let cellId = 0; cellId < cellCount; cellId += 1) {
    const incoming = totalIncoming[cellId] ?? 0;
    const dominant = dominantIncoming[cellId] ?? 0;
    confluence[cellId] = incoming > dominant ? incoming - dominant : 0;
  }
...
export const computePackCellSuitability = (
  ...
  for (let packId = 0; packId < packCellCount; packId += 1) {
    ...
    if (coast === 1) {
      ...
      else if (group === lakeGroupCode.dry) score += rankScoreMap.dry;
      else {
...
const computeRankComputationContext = (
  ...
  positiveFlux.sort((left, right) => left - right);
  const flowMidpoint = Math.floor(positiveFlux.length / 2);
  const meanFlux =
    positiveFlux.length === 0
      ? 0
      : positiveFlux.length % 2 === 0
        ? ((positiveFlux[flowMidpoint - 1] ?? 0) +
...
const isInhabitedPackCell = (
  ...
  return (
    (context.world.cellsFeature[gridCellId] ?? 0) === 1 &&
    (context.world.cellsCulture[gridCellId] ?? 0) > 0
  );
};
...
export const runBurgGenerationStage = (context: GenerationContext): void => {
  ...
  const suitability = computeRankedSuitability(context, true);
  const populatedPackIds = Array.from(
    { length: packCellCount },
    (_, packId) => packId,
  ).filter((packId) => {
    if (!isInhabitedPackCell(context, packId)) return false;
    return (suitability[packId] ?? 0) > 0;
  });
  ...
  let capitals: number[] = [];
  const occupiedCells = new Uint8Array(context.world.cellCount);
  let spacing =
  ...
  while (capitals.length < capitalsNumber && spacing > 1) {
    ...
    for (const packId of capitalOrder) {
      const cellId = packToGrid[packId] ?? 0;
      if ((occupiedCells[cellId] ?? 0) === 1) continue;
      if (hasPackBurgNear(context, capitals, packId, spacing)) continue;
      capitals.push(packId);
      occupiedCells[cellId] = 1;
  ...
  occupiedCells.fill(0);
  for (const packId of burgPackIds) {
    occupiedCells[packToGrid[packId] ?? 0] = 1;
  }
  while (
    ...
    for (const packId of townOrder) {
      ...
      if (hasPackBurgNear(context, burgPackIds, packId, minSpacing)) continue;
  ...
  const selectedBurgPackIds = new Uint32Array(burgCount + 1);
  ...
  for (let index = 0; index < burgCount; index += 1) {
    ...
    selectedBurgPackIds[burgId] = packId;
  ...
  for (const [waterbodyId, burgIds] of portCandidates.entries()) {
    ...
    for (const burgId of burgIds) {
      ...
      const packId = selectedBurgPackIds[burgId] ?? -1;
      const haven = packId >= 0 ? (packHaven[packId] ?? -1) : -1;
      if (haven < 0) continue;
  ...
  context.internal.burgPackIds = selectedBurgPackIds;
};
...
export const runStatesStage = (context: GenerationContext): void => {
  ...
  for (let index = 0; index < capitalBurgIds.length; index += 1) {
    ...
    const centerPack =
      context.internal.burgPackIds[burgId] ?? gridToPack[centerCell] ?? -1;
